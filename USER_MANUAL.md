...
16. [Workflow โดยละเอียด — ทุกกระบวนการ](#16-workflow-โดยละเอียด--ทุกกระบวนการ)
...
## 16. Workflow โดยละเอียด — ทุกกระบวนการ
...
### 16.1 กระบวนการ: เข้าร่วมระบบครั้งแรก (User Onboarding)

```
┌────────────────────────────────────────────────────────────────┐
│  กระบวนการ: User Onboarding                                    │
│  ผู้เกี่ยวข้อง: ผู้ใช้ใหม่ + 🔴 MasterAdmin                    │
└────────────────────────────────────────────────────────────────┘

ขั้นที่ 1 │ ผู้ใช้ใหม่
          │  ► เปิด https://cmg-cdms-doccontrol.web.app
          │  ► กด "Sign in with Google" หรือ Email/Password
          │  ► ระบบสร้าง User Document อัตโนมัติ:
          │      status: "pending", role: "Viewer", isActive: false
          ▼
ขั้นที่ 2 │ ผู้ใช้ใหม่
          │  ► เห็นหน้า "Pending Approval"
          │  ► ไม่สามารถเข้าใช้งานได้จนกว่าจะได้รับการ Approve
          ▼
ขั้นที่ 3 │ 🔴 MasterAdmin
          ...
          ▼
ขั้นที่ 4 │ ระบบ
          │  ► อัพเดท status: "active", isActive: true
          │  ► ผู้ใช้ใหม่เข้าระบบได้ทันที (Real-time)
          ▼
สิ้นสุด   │  ผู้ใช้ใหม่เริ่มใช้งานได้ตาม Role ที่กำหนด
```

---

### 16.2 กระบวนการ: สร้างโครงการใหม่ (Create Project)
...
ขั้นที่ 1 │ 🔴🔵🟢 ผู้มีสิทธิ์
          ...
          ▼
ขั้นที่ 2 │ ระบบ
          │  ► ตรวจสอบ Edit Lock (ป้องกัน 2 คนสร้างพร้อมกัน)
          │  ► บันทึก Project Document ลง Firestore:
          │      memberIds: [UID ผู้สร้าง]
          │      roles: {UID: "Admin"}
          │      createdAt: timestamp
          ▼
ขั้นที่ 3 │ ระบบ
          │  ► Project ใหม่ปรากฏในรายการ My Projects ทันที
          │  ► Project Selector ใน Top Bar อัพเดทอัตโนมัติ
          ▼
...
### 16.3 กระบวนการ: รับเอกสารจาก Client/หน่วยงานภายนอก (Transmittal In)
...
ขั้นที่ 2 │ 🔵🟢🟡 กรอกข้อมูล
          ...
          │  ► Subject *            หัวข้อย่อ
          │  ► Purpose              For Approval / For Action /
          │                         For Information / For Record
          │  ► Status               Submitted (default)
          │  ► Date                 วันที่รับจริง
          │  ► แนบไฟล์              อัพโหลด PDF / เอกสาร
          ▼
ขั้นที่ 3 │ ระบบ
          │  ► บันทึก requiresReply = true
          │      (เฉพาะ For Approval หรือ For Action)
          │  ► Transmittal ปรากฏในตารางทันที (Real-time)
          │  ► Dashboard: ตัวนับ "Transmittals In" เพิ่มขึ้น
...
ขั้นที่ 4 │ 🔵🟢 ผู้รับผิดชอบ (ถ้ามีไฟล์แนบ)
          ...
          ▼
สิ้นสุด   │  เอกสารถูกบันทึกในระบบ พร้อมติดตาม
```

---

### 16.4 กระบวนการ: ส่งเอกสารออก (Transmittal Out)
...
ขั้นที่ 2 │ 🟡🟢 กรอกข้อมูล
          ...
          │  ► Subject *        หัวข้อ
          │  ► Purpose          For Approval (ขออนุมัติ) /
          │                     For Action (ให้ดำเนินการ) /
          │                     For Information (แจ้งทราบ) /
          │                     For Record (เก็บบันทึก)
          │  ► Status           Submitted (default)
          │  ► Date             วันที่ส่งจริง
          │  ► แนบไฟล์          อัพโหลดไฟล์ที่ส่ง
          ▼
ขั้นที่ 3 │ ระบบ
          │  ► บันทึก requiresReply = true
...
ขั้นที่ 4 │ 🟡🟢 ผู้ส่ง
          ...
          ▼
สิ้นสุด   │  บันทึกการส่งเอกสารในระบบเรียบร้อย
```

---

### 16.5 กระบวนการ: บันทึกเอกสารในทะเบียน (Add Document)
...
ขั้นที่ 2 │ 🟡 กรอกข้อมูล
          ...
          │                     Material Approval / Method Statement /
          │                     Report / Correspondence / Other
          │  ► Status          Draft / Submitted / Under Review
          │  ► Transmittal ID  (ไม่บังคับ) เพื่อ link กับ Transmittal
          │  ► แนบไฟล์          PDF หรือไฟล์เอกสาร
          ▼
ขั้นที่ 3 │ ระบบ
          │  ► บันทึก isLatest: true
          │  ► ปรากฏในตาราง Document Register ทันที
          │  ► Dashboard: "Total Documents" เพิ่มขึ้น
          ▼
สิ้นสุด   │  เอกสารถูกบันทึกเป็น Rev.00 พร้อมติดตามสถานะ
```

---

### 16.6 กระบวนการ: ตรวจสอบและอนุมัติเอกสาร (Review & Approve)

นี่คือกระบวนการหลักของระบบ — ใช้เมื่อมีการส่งเอกสารเพื่อขอ Approval
...
└────────────────────────────────────────────────────────────────┘
  ...
  │                                                              │
ขั้นที่ 1 │ 🟡 Engineer                                          │
          │  ► บันทึกเอกสารใน Document Register (Rev.00)         │
          │  ► สร้าง Transmittal Out, purpose: "For Approval"    │
          │  ► แนบไฟล์เอกสาร                                     │
          │  ► ส่งไฟล์ไปให้ผู้ตรวจสอบ (Email/ส่งตรง)            │
          └──────────────────────────────────────────────────────┘
                              │
                              ▼
  ┌── ฝั่ง Client / ผู้ตรวจสอบ ────────────────────────────────┐
  │                                                              │
ขั้นที่ 2 │ ผู้ตรวจสอบ (ภายนอกระบบ)                              │
          │  ► รับเอกสาร ตรวจสอบ                                 │
          │  ► ส่งคำตอบกลับพร้อม Status Code A/B/C/D             │
          └──────────────────────────────────────────────────────┘
                              │
                              ▼
...
ขั้นที่ 4 │ 🔵 Admin / 🟢 Manager                                 │
          ...
          │    │ Status Code B → Approved as Noted           │   │
          │    │ Status Code C → Revise and Resubmit         │   │
          │    │ Status Code D → Rejected                    │   │
          │    └─────────────────────────────────────────────┘   │
          └──────────────────────────────────────────────────────┘
                              │
              ┌───────────────┴──────────────────┐
              ▼                                  ▼
   ┌── Code A หรือ B ──┐             ┌── Code C (แก้ไข) ──┐
   │ อนุมัติ → จบ       │             │ Revise & Resubmit  │
   │                   │             │                    │
   │ Document status   │             │ ► Rev.00 → Superseded
   │ → Approved        │             │ ► สร้าง Rev.01 ใหม่│
   │ Transmittal       │             │ ► Engineer แก้ไข   │
   │ status → Closed   │             │   และส่งใหม่       │
   └───────────────────┘             │   (วนกลับขั้นที่ 1)│
                                     └────────────────────┘
...
### 16.7 กระบวนการ: แก้ไขและส่งเอกสารใหม่ (Revision Cycle)
...
ขั้นที่ 2 │ 🔵 Admin / 🟢 Manager
          ...
          ▼
ขั้นที่ 3 │ 🟡 Engineer
          │  ► แก้ไขเอกสารตาม Comment ที่ได้รับ
          │  ► อัพโหลดไฟล์ใหม่ (Rev.01) เข้า Document ใหม่
          ▼
ขั้นที่ 4 │ 🟡 Engineer
          │  ► สร้าง Transmittal Out ใหม่
          │      ระบุว่าเป็นการส่ง Rev.01
          │      แนบไฟล์ Rev.01
          ▼
ขั้นที่ 5 │ วนกลับสู่ขั้นตอนตรวจสอบ (16.6)
          │  จนกว่าจะได้ Code A หรือ B
          ▼
สิ้นสุด   │  Document Rev.ล่าสุด ได้รับ Code A/B → Approved
```

---

### 16.8 กระบวนการ: อัพเดทสถานะ Transmittal (Close Transmittal)
...
เงื่อนไขที่ควร Close Transmittal:
  ► เอกสารได้รับการอนุมัติแล้ว (Code A หรือ B)
...
ขั้นที่ 1 │ 🔵 Admin / 🟢 Manager
          ...
          ▼
ขั้นที่ 2 │ ระบบ
          │  ► Transmittal แสดง Badge "Closed" (สีเขียว)
          │  ► Dashboard: "Pending Reply" ลดลง
          │      (requiresReply=true && status ≠ Closed)
          ▼
สิ้นสุด   │  Transmittal ปิดแล้ว ไม่นับใน Pending Reply อีก
```

---

### 16.9 กระบวนการ: แก้ไขข้อมูลโครงการ (Edit Project)
...
ขั้นที่ 1 │ 🔴🔵🟢 ผู้มีสิทธิ์
          ...
          ▼
ขั้นที่ 2 │ ระบบ
          │  ► ฟอร์มฝั่งซ้ายเปลี่ยนเป็น "Edit Project" (สีส้ม)
...
ขั้นที่ 3 │ 🔴🔵🟢 ผู้แก้ไข
          ...
          ▼
ขั้นที่ 4 │ ระบบ
          │  ► updateDoc ลง Firestore
          │  ► แสดง "Project updated successfully!"
          │  ► ฟอร์มกลับเป็น Create mode
          │  ► Card อัพเดทชื่อทันที
          ▼
สิ้นสุด   │  โครงการถูกแก้ไขเรียบร้อย

หมายเหตุ: กด "Cancel" ได้ตลอดเวลาเพื่อยกเลิกโดยไม่บันทึก
```

---

### 16.10 กระบวนการ: ลบโครงการ (Delete Project)
...
ขั้นที่ 3 │ 🔴🔵🟢 ผู้มีสิทธิ์
          ...
          ▼
ขั้นที่ 4 │ ระบบ
          │  ► deleteDoc จาก Firestore
          │  ► โครงการหายออกจากรายการทันที
          │  ► ถ้าโครงการที่ลบ = Active project ปัจจุบัน
          │    → ระบบเปลี่ยนไปใช้โครงการถัดไปอัตโนมัติ
          ▼
สิ้นสุด   │  ⚠️ Transmittal และ Document เดิมยังอยู่ใน Firestore
          │     (ต้องลบด้วยตนเองใน Firebase Console ถ้าต้องการ)
```

---

### 16.11 กระบวนการ: จัดการ User (แก้ไข Role / ปิดใช้งาน)

```
┌────────────────────────────────────────────────────────────────┐
│  กระบวนการ: จัดการ User                                       │
│  ผู้เกี่ยวข้อง: 🔴 MasterAdmin                                  │
└────────────────────────────────────────────────────────────────┘

─── การเปลี่ยน Role ───────────────────────────────────────────

ขั้นที่ 1 │ 🔴 MasterAdmin
          │  ► เข้าหน้า User Management (/users)
          │  ► ค้นหา User ที่ต้องการ
          │  ► เปลี่ยน Dropdown Role:
          │      MasterAdmin / Admin / Manager / Engineer / Viewer
          ▼
ขั้นที่ 2 │ ระบบ
          │  ► updateDoc role ลง Firestore ทันที
          │  ► User เห็นการเปลี่ยนแปลงใน Sidebar ทันที (Real-time)
          │  ► เมนูที่เข้าถึงได้เปลี่ยนตาม Role ใหม่ทันที

─── การ Disable User ──────────────────────────────────────────

ขั้นที่ 1 │ 🔴 MasterAdmin
          │  ► กดปุ่ม "Disable" ข้าง User ที่ต้องการ
          ▼
ขั้นที่ 2 │ ระบบ
          │  ► อัพเดท status: "disabled", isActive: false
          │  ► User ถูก Logout และเข้าระบบไม่ได้

─── การ Re-enable User ────────────────────────────────────────

ขั้นที่ 1 │ 🔴 MasterAdmin
          │  ► กดปุ่ม "Re-enable" ข้าง User ที่ถูก Disable
          ▼
ขั้นที่ 2 │ ระบบ
          │  ► อัพเดท status: "active", isActive: true
          │  ► User เข้าระบบได้อีกครั้งทันที

─── การกำหนดโครงการ ───────────────────────────────────────────

ขั้นที่ 1 │ 🔴 MasterAdmin
          │  ► กดปุ่ม "Projects" Dropdown ของ User
          │  ► ติ๊ก/ยกเลิก Checkbox โครงการ
          ▼
ขั้นที่ 2 │ ระบบ
          │  ► บันทึก assignedProjectIds[] ลง Firestore ทันที
          │  ► User เห็นโครงการที่กำหนดให้ทันที (Real-time)
          │  หมายเหตุ: MasterAdmin/Admin เห็นทุกโครงการอัตโนมัติ
```
...
### 16.13 Swimlane Diagram: กระบวนการขออนุมัติเอกสารครบวงจร

```
╔═══════════════╦══════════════════╦═══════════════════╦═════════════════╗
║   Engineer    ║  Admin/Manager   ║  Document System  ║     Client      ║
╠═══════════════╬══════════════════╬═══════════════════╬═════════════════╣
║               ║                  ║                   ║                 ║
║  สร้างเอกสาร  ║                  ║                   ║                 ║
║  Doc Register ║                  ║                   ║                 ║
